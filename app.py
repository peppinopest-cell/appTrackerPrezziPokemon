...
def scrape_price(url, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"🌐 Tentativo {attempt + 1}/{max_retries} - status {response.status_code}")
...
def job_check_prices():
    ...
    try:
        ...
        if not rows:
            ...
            send_telegram_message("ℹ️ TEST JOB: watchlist vuota, scheduler attivo.")
            return
        ...
        for row in rows:
            ...
            if new_price is not None:
                if old_price is None or new_price != old_price:
                    msg = (
                        f"🚨 AGGIORNAMENTO PREZZO!\n"
                        f"🃏 {nome}\n"
                        f"💶 Nuovo prezzo: {new_price}€ (era {old_price}€)\n"
                        f"🔗 {url}"
                    )
                    ...
                    cur_update.execute(
                        "UPDATE watchlist SET last_price=? WHERE id=?",
                        (new_price, watch_id)
                    )
                    conn.commit()
                ...
                msg = (
                    f"⚠️ TEST JOB FALLITO\n"
                    f"🃏 {nome}\n"
                    f"❌ Prezzo non trovato\n"
                    f"🔗 {url}"
                )
                send_telegram_message(msg)
...
def run_scheduler():
    ...
    print("⏰ Scheduler avviato: controllo ogni 1 minuto")
    while True:
